...
async function main(): Promise<void> {
  const maybeHarnessConfig = buildRejoinHarnessConfig(process.argv.slice(2));
  if ("help" in maybeHarnessConfig) {
    console.log(maybeHarnessConfig.text);
    return;
  }

  const { runtimeConfig: config, rejoinShaping } = maybeHarnessConfig;
  ...
  const serializedRejoinShaping = JSON.stringify(
    serializeRejoinShapingConfig(rejoinShaping),
  );
    ...
    `${JSON.stringify(
      {
        ...
        rejoinShaping: serializeRejoinShapingConfig(rejoinShaping),
      },
      null,
      2,
    )}\n`,
  ...
  log(`starting rejoin-aware deployment runtime in ${artifacts.runDir}`);
  log(
    [
  ...
  log(`rejoinShaping=${formatRejoinShapingSummary(rejoinShaping)}`);
  ...
  const nodeChildren = runtimeConfig.nodeIds.map((nodeId) => {
    ...
    const childInfo = spawnChild({
      name: nodeId,
      script: siblingScriptPath("deployment-node-rejoin"),
      serializedConfig,
      ...
      extraEnv: {
        ...
        RUNTIME_REJOIN_SHAPING: serializedRejoinShaping,
      },
    });
  ...
  try {
    ...
    writeFallbackSummary(runtimeConfig, rejoinShaping, error);
    process.exitCode = 1;
  }
}
...
function writeFallbackSummary(
  ...
  const summary = {
    ...
    config: {
      ...
      model: "deployment_local_tcp_rejoin",
    ...
    fallback: {
      synthesizedBy: "deployment-runtime-rejoin",
